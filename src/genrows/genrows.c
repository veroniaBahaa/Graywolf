...
BOOL initial ;
{
    ...
    if(fp2)
    {
       fprintf(fp2,"%d",row_length);
    }
   if( initial ){
        //diff = row_length/0.7 - row_length;
        //while(diff%480)
          //  diff++;
	stdcell_lengthS = row_length/*+diff*/ ;
        if (fixed_cells_area > 0)
        {
             sprintf( YmsgG,"fixed_cells_area is %d ", fixed_cells_area);
             stdcell_lengthS = fixed_cells_area;
        }
    } 
    else {
...
BOOL snap_core2( l, r, b, t )
INT *l, *r, *b, *t ;
{
    unsigned int width ;   /* new width of core */
    unsigned int height ;   /* new width of core */
    INT numtiles ; /* number of tiles with rows */
    INT total_sep ; /* total row separation */
    TILE_BOX *tileptr ; /* current tile */
    ...
    printf("total_area before: %d\n",total_area);
    total_area = total_area/utilization;
    printf("aspect_ratio: %lf\n",aspect_ratio);
    printf("total_area: %d\n",total_area);
    printf("utilization: %lf\n",utilization);
    height= pow( (total_area * aspect_ratio) , 0.5)   ;
    /*while(1)
    {
       if(height%actual_row_heightS)
          height++;
       else
          break;
    }*/
